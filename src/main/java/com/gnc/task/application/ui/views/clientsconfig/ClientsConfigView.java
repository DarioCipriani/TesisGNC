package com.gnc.task.application.ui.views.clientsconfig;
...
import com.gnc.task.application.data.service.PerteneceService;
import com.gnc.task.application.utilities.CrudOpenEditor;
...
public class ClientsConfigView extends HorizontalLayout {
	...
	private CrudOpenEditor pertenece;
	...
	public ClientsConfigView(AuthenticatedUser authenticatedUser, ClientsService clientsService,
			ClientDetailsServiceImpl clientDetailsServiceImpl, PerteneceService perteneceService) {
		this.authenticatedUser = authenticatedUser;
		this.clientsService = clientsService;
		this.perteneceService = perteneceService;
		...
		if (userLogged.isPresent() && userLogged.get().getRoles().contains(Role.ADMIN)) {
			...
			clientsGrid.addColumn(Cliente::getEmail).setHeader("Email").setResizable(true).setKey("email")
					.setSortable(true);
			//enlaza los datos que estan en la base de datos y lo pone en el grid apenas se abre el menu de clientes
	...
	private void editor(String titulo) {
		...
		//pertenece = new CrudOpenEditor();

		//Creacion del titulo del PopUp
		H2 headline = new H2(titulo);
		headline.getStyle().set("margin-top", "0");
		//editorDiv.add(headline, nombre, apellido, dni, direccion, telefono, email, pertenece);
